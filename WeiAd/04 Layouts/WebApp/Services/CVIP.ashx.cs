...
namespace WebApp.Services
{
    ...
    /// CVIP 的摘要说明
    /// </summary>
    public class CVIP : IHttpHandler
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                //mlog = string.Format("{0}-{1}-{2}-{3}-{4}", pname, curl, hisurl, adid, filepath);
                //DN.Framework.Utility.LogHelper.Write(mlog, "error1");
                ...
                if (DN.WeiAd.Business.Config.AppConfig.IpAreas.Count != 0 && (!string.IsNullOrEmpty(log.Region)))
                {
