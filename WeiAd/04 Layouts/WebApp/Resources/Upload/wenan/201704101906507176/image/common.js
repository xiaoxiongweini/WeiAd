...
var img = {
	...
	errorhandle : function(obj, is_err_t) {
		...
		parentnode.append('<div class="loading" style="background:url('+ IMGDIR +'images/imageloading.gif) no-repeat center center;width:'+parentnode.width()+'px;height:'+parentnode.height()+'px"></div>');
...
var dialog = {
	init : function() {
		$(document).on('click', '.dialog', function() {
			var obj = $(this);
			popup.open('<img src="' + IMGDIR + 'images/imageloading.gif">');
			$.ajax({
				type : 'GET',
...
var pullrefresh = {
	init : function() {
		...
		.on('touchend', function(e) {
			if(status == true) {
				if(reloadflag) {
					contentobj.html('<img src="'+ STATICURL + 'images/icon_load.gif" style="vertical-align:middle;margin-right:5px;">正在加载...');
