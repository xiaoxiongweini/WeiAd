...
namespace WebApp.Admin.Ads
{
    public partial class AdPageList : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if(!string.IsNullOrEmpty(id))
                {
                    if(isdel == "1")
                    {
                        AdPageInfoBLL.Instance.Delete(new AdPageInfoPara() { Id = int.Parse(id) });
                    }
                }
        ...
        {          
            ...
            {
                string txt = string.Format("{0}-{1}-{2}-{3}", item.Id, item.UserName, item.NickName, AccountInfoBLL.Instance.GetUserTypeNameById(item.UserType));
