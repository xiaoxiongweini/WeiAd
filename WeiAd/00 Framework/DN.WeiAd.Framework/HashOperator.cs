...
namespace DN.WeiAd.Framework
{
    ...
    public class HashOperator : RedisOperatorBase
    {
        ...
        {  
            var value = DN.Framework.Utility.Serializer.SerializeObject(t);
        ...
        public T Get<T>(string hashId, string key)
        {
            ...
            if (value != null)
            {
                return DN.Framework.Utility.Serializer.DeserializeObject<T>(value);
            }
            return default(T);
        }
        /// <summary>  
        ...
        public List<T> GetAll<T>(string hashId)
        {
            ...
            if (list != null && list.Count > 0)
            {
                list.ForEach(x =>
                {
                    var value = DN.Framework.Utility.Serializer.DeserializeObject<T>(x);
                    result.Add(value);
                });
            }
            return result;
