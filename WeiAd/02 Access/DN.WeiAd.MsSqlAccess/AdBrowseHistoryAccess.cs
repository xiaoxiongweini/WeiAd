...
{
    public partial class AdBrowseHistoryAccess : AdBrowseHistoryInterface
    {
        #region SQL
        ...
        /// 新增
        /// </summary>
        const string INSERT = "INSERT INTO AdBrowseHistory ([Url],[ClientIp],[BrowseType],[CreateDate],[AdId],[AdUserId],[FlowUserId],[AdUrl],[Money],[IsMoney],[Time],[ClientId],[IsMobile],[ReferrerUrl],[BrowseName],[BrowseVersion],[OsName]) VALUES (@Url,@ClientIp,@BrowseType,@CreateDate,@AdId,@AdUserId,@FlowUserId,@AdUrl,@Money,@IsMoney,@Time,@ClientId,@IsMobile,@ReferrerUrl,@BrowseName,@BrowseVersion,@OsName)";

        /// <summary>
        /// 修改
        /// </summary>
        const string EDIT = "UPDATE AdBrowseHistory SET [Url]=@Url,[ClientIp]=@ClientIp,[BrowseType]=@BrowseType,[CreateDate]=@CreateDate,[AdId]=@AdId,[AdUserId]=@AdUserId,[FlowUserId]=@FlowUserId,[AdUrl]=@AdUrl,[Money]=@Money,[IsMoney]=@IsMoney,[Time]=@Time,[ClientId]=@ClientId,[IsMobile]=@IsMobile,[ReferrerUrl]=@ReferrerUrl,[BrowseName]=@BrowseName,[BrowseVersion]=@BrowseVersion,[OsName]=@OsName WHERE [Id]=@Id";

        /// <summary>
        /// 删除
        /// </summary>
        const string DELETE = "DELETE FROM AdBrowseHistory";

        /// <summary>
        /// 加载数据
        /// </summary>
        const string LOAD = "SELECT * FROM AdBrowseHistory @WHERE @ORDER";

        /// <summary>
        /// 分页查询
        /// </summary>
        const string QUERYPAGE = "SELECT TOP @PAGESIZE * FROM [AdBrowseHistory] @WHERE AND Id NOT IN (SELECT TOP @PTOP Id FROM [AdBrowseHistory] @WHERE @ORDER)  @ORDER";

        /// <summary>
        /// 查询记录数
        /// </summary>
        const string QUERYCOUNT = "SELECT COUNT(1) FROM AdBrowseHistory";


        #endregion

        public override bool Delete(AdBrowseHistoryPara mp)
        {
        ...
        public override bool Edit(AdBrowseHistoryVO m)
        {
            CodeCommand command = new CodeCommand();
        ...
        public override string GetConditionByModel(AdBrowseHistoryVO m)
        {
            return "";
        }

        public override string GetConditionByPara(AdBrowseHistoryPara mp)
        {
        ...
        public override List<AdBrowseHistoryVO> GetModels(ref AdBrowseHistoryPara mp)
        {
            string where = GetConditionByPara(mp);
            ...
            List<AdBrowseHistoryVO> list = new List<AdBrowseHistoryVO>();

            for (int i = 0; i < table.Rows.Count; i++)
            {
                list.Add(new AdBrowseHistoryVO(table.Rows[i]));
            }
        ...
        public override List<AdBrowseHistoryVO> GetModels(AdBrowseHistoryPara mp)
        {
            string where = GetConditionByPara(mp);
        ...
        public override string GetOrderByModel(AdBrowseHistoryVO m)
        {
            return "";
        }

        public override string GetOrderByPara(AdBrowseHistoryPara mp)
        {
        ...
        public override string GetOtherConditionByModel(AdBrowseHistoryVO m)
        {
            return "";
        }

        public override string GetOtherConditionByPara(AdBrowseHistoryPara mp)
        {
            return "";
        }

        public override int GetRecords(AdBrowseHistoryPara mp)
        {
            string where = GetConditionByPara(mp);
        ...
        public override AdBrowseHistoryVO GetSingle(AdBrowseHistoryPara mp)
        {
        ...
        public override bool Insert(AdBrowseHistoryVO m)
        {
            CodeCommand command = new CodeCommand();
        ...
        public override int InsertIdentityId(AdBrowseHistoryVO m)
        {
            CodeCommand command = new CodeCommand();
