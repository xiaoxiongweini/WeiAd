...
{
    public partial class LogBrowseHistoryAccess : LogBrowseHistoryInterface
    {
        #region SQL
        ...
        /// 新增
        /// </summary>
        const string INSERT = "INSERT INTO LogBrowseHistory ([Url],[ClientIp],[BrowseType],[CreateDate],[AdId],[AdUserId],[FlowUserId],[AdUrl],[Money],[IsMoney],[Time],[ClientId],[IsMobile],[ReferrerUrl],[BrowseName],[BrowseVersion],[OsName],[Country],[Area],[Region],[City],[County],[Isp],[IpSource]) VALUES (@Url,@ClientIp,@BrowseType,@CreateDate,@AdId,@AdUserId,@FlowUserId,@AdUrl,@Money,@IsMoney,@Time,@ClientId,@IsMobile,@ReferrerUrl,@BrowseName,@BrowseVersion,@OsName,@Country,@Area,@Region,@City,@County,@Isp,@IpSource)";

        /// <summary>
        /// 修改
        /// </summary>
        const string EDIT = "UPDATE LogBrowseHistory SET [Url]=@Url,[ClientIp]=@ClientIp,[BrowseType]=@BrowseType,[CreateDate]=@CreateDate,[AdId]=@AdId,[AdUserId]=@AdUserId,[FlowUserId]=@FlowUserId,[AdUrl]=@AdUrl,[Money]=@Money,[IsMoney]=@IsMoney,[Time]=@Time,[ClientId]=@ClientId,[IsMobile]=@IsMobile,[ReferrerUrl]=@ReferrerUrl,[BrowseName]=@BrowseName,[BrowseVersion]=@BrowseVersion,[OsName]=@OsName,[Country]=@Country,[Area]=@Area,[Region]=@Region,[City]=@City,[County]=@County,[Isp]=@Isp,[IpSource]=@IpSource WHERE [Id]=@Id";

        /// <summary>
        /// 删除
        /// </summary>
        const string DELETE = "DELETE FROM LogBrowseHistory";

        /// <summary>
        /// 加载数据
        /// </summary>
        const string LOAD = "SELECT * FROM LogBrowseHistory @WHERE @ORDER";

        /// <summary>
        /// 分页查询
        /// </summary>
        const string QUERYPAGE = "SELECT TOP @PAGESIZE * FROM [LogBrowseHistory] @WHERE AND Id NOT IN (SELECT TOP @PTOP Id FROM [LogBrowseHistory] @WHERE @ORDER)  @ORDER";

        /// <summary>
        /// 查询记录数
        /// </summary>
        const string QUERYCOUNT = "SELECT COUNT(1) FROM LogBrowseHistory";


        #endregion

        public override bool Delete(LogBrowseHistoryPara mp)
        {
        ...
        public override string GetConditionByModel(LogBrowseHistoryVO m)
        {
            return "";
        }

        public override string GetConditionByPara(LogBrowseHistoryPara mp)
        {
        ...
        public override List<LogBrowseHistoryVO> GetModels(ref LogBrowseHistoryPara mp)
        {
            string where = GetConditionByPara(mp);
            ...
            List<LogBrowseHistoryVO> list = new List<LogBrowseHistoryVO>();

            for (int i = 0; i < table.Rows.Count; i++)
            {
                list.Add(new LogBrowseHistoryVO(table.Rows[i]));
            }
        ...
        public override List<LogBrowseHistoryVO> GetModels(LogBrowseHistoryPara mp)
        {
            string where = GetConditionByPara(mp);
        ...
        public override string GetOrderByModel(LogBrowseHistoryVO m)
        {
            return "";
        }

        public override string GetOrderByPara(LogBrowseHistoryPara mp)
        {
        ...
        public override string GetOtherConditionByModel(LogBrowseHistoryVO m)
        {
            return "";
        }

        public override string GetOtherConditionByPara(LogBrowseHistoryPara mp)
        {
            return "";
        }

        public override int GetRecords(LogBrowseHistoryPara mp)
        {
            string where = GetConditionByPara(mp);
        ...
        public override LogBrowseHistoryVO GetSingle(LogBrowseHistoryPara mp)
        {
        ...
        public override bool Insert(LogBrowseHistoryVO m)
        {
            CodeCommand command = new CodeCommand();
