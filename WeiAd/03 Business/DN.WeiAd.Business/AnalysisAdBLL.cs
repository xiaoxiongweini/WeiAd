...
namespace DN.WeiAd.Business
{
    ...
    class AnalysisAdBLL
    {
        ...
        static AnalysisAdBLL m_proxy = null;

        static ChartInterface acc = null;
        public static AnalysisAdBLL Instance
        {
            get
            {
                if (m_proxy == null)
...
group by time,AdId", flow.Time.ToString("yyyyMMdd"), sb);
        ...
        /// <param name="flow"></param>
        /// <returns></returns>
        public DataTable GetAllAdUserDetail(FlowInfo flow)
        {
            StringBuilder sb = new StringBuilder();
...
from    [AdBrowse]
where time={0} {1}
group by time,AdUserId", flow.Time.ToString("yyyyMMdd"), sb);
            ChartPara cp = new ChartPara();
            cp.CommandText = cmd;
            DataTable table = acc.GetTable(cp);
